...
if __name__ == '__main__':
    ...
    draw.save('00_02_why_learn_1.png')

    # image 3
    figure, axis = draw.init()
    draw.title(axis, '课程介绍')
    draw.subtitle(axis, '2. 为什么要学？')
    draw.text(axis, 'AlphaGo 掌握了古老的围棋游戏，击败了围棋世界冠军，并开启了人工智能系统的新时代。', index=0)
    draw.text(axis, 'AlphaFold 揭示了数百万种复杂的 3D 蛋白质结构，并帮助科学家了解生命分子如何相互作用。', index=2)
    draw.text(axis, 'Telsa FSD 自动驾驶系统，旨在实现车辆从起点到目的地的全程自主驾驶。', index=4)
    draw.text(axis, 'Github Copilot 全球最广泛采用的人工智能开发工具。', index=6)
    draw.text(axis, 'ChatGPT 能够理解并生成自然语言，用于回答问题、提供建议和进行互动交流。', index=8)
    draw.text(axis, 'Apple Intelligence 帮助用户轻松书写、表达自我和完成工作。', index=10)
    draw.text(axis, 'Midjourney 根据用户的文本描述生成高度创意的艺术作品和视觉图像。', index=12)
    draw.save('00_02_why_learn_2.png')

    # image 3
    figure, axis = draw.init()
    draw.title(axis, '课程介绍')
    draw.subtitle(axis, '3. 机器学习系列')
    ...
    draw.save('00_03_machine_learning_series.png')
